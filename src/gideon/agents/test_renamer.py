...
def test_generate_filename_all_unknowns(wizard):
    doc = DocumentInfo([], "", "", "")
    filename = wizard.generate_filename(doc)
    assert filename.startswith(f"{UNKNOWN_AUTHOR}.{UNKNOWN_TITLE}.{UNKNOWN_TOPIC}.") or filename.startswith(f"{UNKNOWN_AUTHOR}..{UNKNOWN_TITLE}.{UNKNOWN_TOPIC}.")
    assert filename.endswith(".pdf")
...
async def test_analyze_document_valid():
    ...
    with patch("src.gideon.agents.renamer.LLMServiceFactory.create", return_value=MagicMock(create_chain=AsyncMock(return_value=mock_chain))):
        wizard = RenameWizard()
